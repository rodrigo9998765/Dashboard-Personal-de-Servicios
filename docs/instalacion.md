...
1. inicializacion de la maquina virtual:
   ```
   podman machine init
   ```
   
2. encender la maquina virtual
   ```
   podman machine start
   ```
   
3. Crear archivo `podman-compose.yml`:
   ```yml
	services:
...
4. Iniciar servicios
	```
	podman compose -f .\podman-compose.yml up --force-recreate -d
	```
